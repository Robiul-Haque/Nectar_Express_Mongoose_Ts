...
import { createCategory, deleteCategory, getAllCategories, getCategoryStats, updateCategory } from "./category.controller";

const router = Router();

router.post("/create", authenticate(["admin"]), upload.single("icon"), validateRequest(createCategorySchema), createCategory);
router.get("/stats", authenticate(["admin"]), getCategoryStats);
